...
In this lab you will use Ironhack's country [API](https://ih-countries-api.herokuapp.com/countries), to create your own Wikipedia of countries!
...
- `CountriesList`: Displays the list of links with the country names. Each link should be a `react-router-dom` `Link` which we will use to <u>send</u> the country code (`alpha3Code`) via the URL. It will act as a navbar for the countries.
...
Create a header component that displays the title _LAB - WikiCountries_.

<br>

### Iteration 2.2 | CountriesList component

This component should receive the list of countries from `App` and render a list of `Link`s, each having a country's `alpha3Code` embedded in the URL. Clicking on a `Link` should render the `CountryDetails` component.
